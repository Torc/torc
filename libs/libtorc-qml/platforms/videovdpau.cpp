...
#include "platforms/nvctrl/torcnvcontrol.h"
...
bool VideoVDPAU::CreateFullVDPAU(void)
{
    ...
    // this must be completed in the Qt render thread
    if (QThread::currentThread()->objectName() != QTRENDER_THREAD)
    {
        ...
        {
            m_callbackLock->lock();
            proxy->RegisterCallback(VDPAUUICallback, this, CreateFull);
...
bool VideoVDPAU::Dereference(void)
{
    if (!IsShared() && !TorcQThread::IsMainThread())
    {
        ...
        TorcQMLEventProxy *proxy = dynamic_cast<TorcQMLEventProxy*>(gLocalContext->GetUIObject());
        if (proxy)
            proxy->RegisterCallback(VDPAUUICallback, this, Destroy);
...
bool VideoVDPAU::CheckHardwareSupport(void)
{
    ...
    if (TorcNVControl::NVControlAvailable(m_display))
        m_featureSet = GetNVIDIAFeatureSet(TorcNVControl::GetPCIID(m_display, DefaultScreen(m_display)) & 0xffff);
...
bool VideoVDPAU::HandlePreemption(void)
{
    ...
    if (!TorcQThread::IsMainThread())
    {
        ...
        {
            m_callbackLock->lock();
            proxy->RegisterCallback(VDPAUUICallback, this, Recreate);
            m_callbackWait.wait(m_callbackLock);
            m_callbackLock->unlock();
        }
