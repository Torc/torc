...
VideoVAAPI* VideoVAAPI::GetVideoVAAPI(AVCodecContext *Context, bool OpenGL)
{
    ...
    if (OpenGL)
    {
        ...
        if (proxy)
        {
            proxy->RegisterCallback(VAAPIUICallback, videovaapi, Create);
            int tries = 0;
...
VideoVAAPI::VideoVAAPI(AVCodecContext *Context, bool OpenGL, AVPixelFormat TestFormat)
  : TorcReferenceCounter(),
    ...
    m_glxSurface(NULL),
    m_surfaceTexture(0),
    m_surfaceTextureType(GL_TEXTURE_2D)
{
    if (m_avContext)
    {
...
bool VideoVAAPI::Dereference(void)
{
    if (m_opengl && !IsShared())
    {
        ...
        TorcQMLEventProxy *proxy = dynamic_cast<TorcQMLEventProxy*>(gLocalContext->GetUIObject());
        if (proxy)
            proxy->RegisterCallback(VAAPIUICallback, this, Destroy);
        return false;
    }
...
bool VideoVAAPI::CopySurfaceToTexture(VideoFrame *Frame, VAAPISurface *Surface, GLuint Texture, GLenum TextureType, VideoColourSpace *ColourSpace)
{
    if (m_state == Context && m_opengl && Surface && Texture)
    {
        ...
        if (!m_glxSurface)
        {
            ...
            VAStatus status = VAAPIGLX.m_createSurfaceGLX(m_vaDisplay, TextureType, Texture, &m_glxSurface);
    ...
    bool UpdateFrame(VideoFrame *Frame, VideoColourSpace *ColourSpace, void *Surface, void *SurfaceType)
    {
        if (Frame && Surface && ColourSpace && Frame->m_pixelFormat == AV_PIX_FMT_VAAPI_VLD && Frame->m_acceleratedBuffer)
        {
            ...
            if (vaapisurface)
            {
                ...
                {
                    if (texture)
                        videovaapi->CopySurfaceToTexture(Frame, vaapisurface, *texture, *type, ColourSpace);
